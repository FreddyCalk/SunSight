...
This document describes the current migrations and backend contracts. Edge
Function operations are covered by [local-runbook.md](local-runbook.md).
...
- `devices`: many devices per profile; push token is globally unique.
  `register_device` refreshes a token only for its existing owner and rejects a
  cross-user token takeover with a uniqueness error.
...
The authenticated client and caller-scoped Edge Function RPC surface includes:
...
- `public.assign_photo_upload_path(...)`
- `public.dispatch_blast(uuid)`
- `public.get_blast_access(uuid)`

`replace_contact_matches` returns `void`; the `match-contacts` Edge Function
returns only `{ "synchronized": true }`. Neither contract exposes a matched
user count that could be used as a registration oracle.

The service-role-only RPC surface includes
`public.complete_photo_blast(...)` and the outbox claim and completion
functions. `complete_photo_blast` also verifies the service role in its body;
authenticated and anonymous roles have no execute grant.

Caller-scoped functions derive identity from `auth.uid()`. All of these
functions use explicit empty `search_path` settings. Recipient selection
currently implements expiring one-way contact matches, active profiles, fresh
and accurate locations, configured distance and cap, bilateral blocks,
disabled blasts, and active mutes. Quiet-hour evaluation is not implemented.

## Blast expiry authority

`public.create_blast` requires an IANA timezone. It computes the next local
midnight and the configured maximum visibility from server time, then stores
the earliest boundary. An optional client `expiresAt` value is only a shortening
hint: the database clamps it to the server-computed boundary and rejects it if
it is already expired. The client is not authoritative for blast lifetime.

## Delivery reclaim and retry

`public.claim_notification_outbox` claims unprocessed work whose lock is absent
or older than five minutes. A worker that stops after claiming can therefore
leave a `queued` delivery with `next_attempt_at = null`; after the lock TTL that
orphaned delivery is eligible to be claimed again. The claim filter skips
accepted and invalid-token deliveries, future scheduled retries, and terminal
failed deliveries whose `next_attempt_at` is null. It does not treat an
orphaned queued row with a null retry time as terminal.

`public.finish_notification_outbox` schedules transient failures with bounded
exponential backoff, clears the outbox lock for retry, and disables terminally
...
`create-blast` issues private signed upload URLs and `get-blast-access` issues
recipient-gated, expiry-checked read URLs for display derivatives.
`complete-photo-upload` currently validates basic byte, MIME/magic, and
dimension bounds and then fails closed with `MEDIA_PROCESSOR_UNAVAILABLE`.
EXIF removal and derivative generation are therefore unavailable, photo blasts
cannot complete, and originals are never exposed to recipients. When a media
processor is implemented, only the Edge Function's service-role client may call
`complete_photo_blast`; end users cannot invoke that state transition directly.
...
- production photo decoding, EXIF stripping, display/thumbnail derivatives, or
  expiry deletion;
- quiet-hours evaluation, report/account-deletion operations, observability, or
  Expo receipt polling; or
- complete mobile blast, capture, and Sky Window flows.
